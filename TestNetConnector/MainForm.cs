...
namespace TestNetConnector
{
    ...
    {
        String _version = "  v2.1[2012/06/05 Rev.2] - 항목들을 View 메뉴에서 드러나거나 숨기게 할 수 있음....";
        ...
        String _saveIpPath = @".\savedIp.dat";
        ...
        public MainForm()
        {
            ...
            StructXMLParser.VariablesList.Add("Index", new VariableInfo("Index", sendIndex));
            ...
            M_EchoModeMsg.Click += new EventHandler(M_EchoMode_Click);
            ...
            _echoModes = new ToolStripMenuItem[] { M_EchoModeReceived, M_EchoModeMsg, M_EchoModeNoEcho, M_EchoModeNext, M_EchoModeSelected };
            ...
            if (File.Exists(filePath))
            {
                ...
                try
                {
                    file = File.OpenRead(_saveIpPath);
                    try
                    {
                        ...
                        while (xReader.Read())
                        {
                            ...
                            {
                                case "ip":
                                    ip = xReader.ReadString();
                                    T_IP.IpAddress = ip;
                                    break;
                                case "port":
                                    port = xReader.ReadString();
                                    T_Port.U_Text = port;
                                    break;
                                case "endian":
                                    _endian = (xReader.ReadString().ToLower().Equals("big")) ? Endians.Big : Endians.Little;
                                    ...
                                    break;
                                case "connpos":
                                    _connPos = (xReader.ReadString().ToLower().Equals("server"))? NetPosition.Server:NetPosition.Client;
                                    ...
                                    break;
                                case "connmode":
                                    _connMode = (xReader.ReadString().ToLower().Equals("tcp"))? ConnMode.Tcp: ConnMode.Udp;
                                    L_ConnType.Text = _connMode.ToString();
                                    M_ConnModeTcp.Checked = R_ConnModeTcp.Checked = (_connMode == ConnMode.Tcp);
                                    M_ConnModeUdp.Checked = R_ConnModeUdp.Checked = (_connMode == ConnMode.Udp);
            ...
            this.FormClosed += new FormClosedEventHandler(TcpServerForm_FormClosed);
        ...
        void M_ViewMsgBox_Click(object sender, EventArgs e)
        {
            M_ViewMsgBox.Checked = !M_ViewMsgBox.Checked;
            G_MsgBox.Visible = M_ViewMsgBox.Checked;
        }
        ...
        {
            _savedList.setEnv(SendMsgToNet, _endian, Ch_IsStringWithNull.Checked, (SavedList.StrEncoding)_encoding);
        ...
        void B_ShowList_Click(object sender, EventArgs e)
        {
            _savedList.setEnv(SendMsgToNet, _endian, Ch_IsStringWithNull.Checked, (SavedList.StrEncoding)_encoding);
            _savedList.Show();
            _savedList.SetBounds(this.Location.X - _savedList.Width, this.Location.Y, 0, 0, BoundsSpecified.Location);
        ...
        SavedList _savedList = new SavedList();
        ...
        public int SendMsgToNet(Byte[] buff, int offset, int size, String text=null)
        {
            if (text == null) text = T_Msg.Text;
                    ...
                    else
                    {
                        for (int k = 1; k < units.Length; k++)
                        {
                            ...
                            else if (typeName == TypeHandling.TypeName.HexString)
                            {
                                ...
                                else if (typeSize == 2) packets.SetValue((short)TypeHandling.getHexNumber<short>(units[k]), k - 1);
                                else if (typeSize == 4) packets.SetValue((int)TypeHandling.getHexNumber<int>(units[k]), k - 1);
                                else packets.SetValue((Int64)TypeHandling.getHexNumber<long>(units[k]), k - 1);
                            }
        ...
        private Array getArray(string type, String[] units, out int typeSize, out Boolean unsigned)
        {
            ...
            else if (type.Equals("char") || type.Equals("string"))
            {
                ...
                if (_encoding == StringEncoding.UTF8) packets = Encoding.UTF8.GetBytes(units[1]);
                else if (_encoding == StringEncoding.UTF16) packets = Encoding.Unicode.GetBytes(units[1]);
                else if (_encoding == StringEncoding.UTF32) packets = Encoding.UTF32.GetBytes(units[1]);
                else if (_encoding == StringEncoding.UTF7) packets = Encoding.UTF7.GetBytes(units[1]);
        ...
        private String getDataString()
        {
            ...
            if (_msgFormat != MsgFormat.String)
            {
                if (_dataTypeSize == -1) //struct
                {
                    ...
                    for (int i = 0; i < _nsReceived.Count; i++)
                    {
                        typeSize = TypeHandling.getTypeSize(_nsReceived.Items[i].TypeString);
                        if (typeSize < 0)
                        {
                            dataBuff = null;
                        }
                        ...
                        {
                            dataBuff = getArray(typeSize, _nsReceived.Items[i].size);
                        }
                        ...
                        dataStr += _nsReceived.Items[i].TypeString;
                        if (_nsReceived.Items[i].TypeString.Equals("string"))
                        {
                            dataStr += ",";
                            unit = "";
                            int strSize = 0;
                            for (int j = structIndex; j < _totalRecvSize; j++)
                            {
                                if (_recvBuff[j] == 0)
                                {
                        ...
                        else
                        {
                            try
                            {
                                if (_endian == Endians.Big)
                                    Swaper.swapWithSize(_recvBuff, dataBuff, typeSize, buffSize, structIndex, 0);
                                else
                                    Buffer.BlockCopy(_recvBuff, structIndex, dataBuff, 0, buffSize);
                            }
                            catch
                            {
                ...
                {
                    Array dataBuff = getArray(_dataTypeSize, _totalRecvSize / _dataTypeSize);

                    if (_endian == Endians.Big) Swaper.swapWithSize(_recvBuff, dataBuff, _dataTypeSize, Buffer.ByteLength(dataBuff));
                    else Buffer.BlockCopy(_recvBuff, 0, dataBuff, 0, Buffer.ByteLength(dataBuff));
            ...
            else
            {
                ...
                {
                    case StringEncoding.UTF7:
                        dataStr = Encoding.UTF7.GetString(_recvBuff, 0, _totalRecvSize);
                        break;
                    case StringEncoding.UTF8:
                        dataStr = Encoding.UTF8.GetString(_recvBuff, 0, _totalRecvSize);
                        break;
                    case StringEncoding.UTF32:
                        dataStr = Encoding.UTF32.GetString(_recvBuff, 0, _totalRecvSize);
                        break;
                    case StringEncoding.UTF16:
                    default:
                        dataStr = Encoding.Unicode.GetString(_recvBuff, 0, _totalRecvSize);
                        break;
                    ...
                    while (xReader.Read())
                    {
                        ...
                        switch (xReader.Name)
                        {
                            ...
                            case "ResponseMode":
                                ...
                                break;
                            case "Msg":
                                if (mode != LoadMode.Config) T_Msg.Text = value;
                                break;
                            case "MsgText":
        ...
        void loadMsgStruct()
        {
            ...
            try
            {
                ...
                file.Close();
                _nsSend.setNativeText(str, _endian == Endians.Big);
                MakeMsg();
                //makeStruct(_nsSend);
            }
            catch(Exception e)
        ...
        enum ConnMode { Tcp = 0, Udp = 1, UdpMulticast=2 };
        ConnMode _connMode = ConnMode.Tcp;
        ...
        void M_ConnSite_Click(object sender, EventArgs e)
        {
            ...
            {
                if (_connMode == ConnMode.Tcp) setTcpServer();
                else setUdpServer();
                //if (T_IP.IpAddress.Equals("127.0.0.1")) T_IP.IpAddress = "0.0.0.0";
            }
            else
            {
                if (_connMode == ConnMode.Tcp) setTcpClient();
                else setUdpClient();
        ...
        enum StringEncoding { UTF8 = 0, UTF7 = 1, UTF16, UTF32, ASCII };
        ...
        enum ResponseMode { Echo = 0, EchoWithMsg = 1, NoEcho, Next, Selected };
        ...
        void M_EchoMode_Click(object sender, EventArgs e)
        {
            ToolStripMenuItem c = (ToolStripMenuItem)sender;
            int tag = Int32.Parse(c.Tag.ToString());
            _responseMode = (ResponseMode)tag;
            CheckAndTheOtherUnCheck(c, _echoModes);
            L_EchoMode.Text = c.Text;
        }
        ...
        void R_AnalysisMsgFormat_Click(object sender, EventArgs e)
        {
            ...
            D_History.AddARow(new object[] { _totalRecvSize.ToString(), _sentIp, getDataString()});// size.ToString(), ip, data });
            _totalRecvSize = 0;
        ...
        private bool runAfterRecv()
        {
            ...
            else
            {
                ...
                if (_responseMode == ResponseMode.Echo)
                {
                    int size = 0;
                    if (_connMode == ConnMode.Tcp)
                    {
                ...
                else if (_responseMode == ResponseMode.EchoWithMsg)
                {
                    if (_isSendMsgOk) SendMsg();
                }
                else if (_responseMode == ResponseMode.Next)
                {
                    isSuccess = _savedList.RunNext();
                }
                else if (_responseMode == ResponseMode.Selected)
                {
                    _savedList.RunSelected();
                }
                else //NoEcho
                {
                }
        ...
        {
            DlgMsgMaker dlgMakeMsg = new DlgMsgMaker(_nsSend, "./msg.txt", _endian == Endians.Big);
        ...
        void MakeMsg()
        {
            ...
            for (int i = 0; i < _nsSend.Count; i++)
            {
                ...
                if (i != 0) msg += "/";
                msg += _nsSend.Items[i].TypeString;
        ...
        private void B_Start_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (_connMode == ConnMode.Tcp)
                {
                    if (_conn.isConnected()) _conn.Close();
                    //Boolean isUsingRecvQueue = (_headerSize > 0) ? false : true;
                    _conn.setServer(T_IP.IpAddress, T_Port.IntValue, true, null, T_Timeout.IntValue);
                    //if (isUsingRecvQueue == false) _tcp.setRecvLoop(RecvLoop);
                ...
                */
                ...
                StreamWriter file = File.CreateText(_saveIpPath);
                XmlWriterSettings setting = new XmlWriterSettings();
                setting.Indent = true;
                setting.IndentChars = "    ";
                setting.NewLineChars = "\r\n";
        ...
        void stopConnection()
        {
            ...
            else
            {
                ...
                if (_connMode == ConnMode.Tcp) G_HeaderDefine.Enabled = true;
            }
        }
        ...
        void TestServerForm_Resize(object sender, EventArgs e)
        {
            ...
            if (SP_Spliter.Orientation == Orientation.Horizontal) P_MenuBarContainer.Width = P_Menues.Width;
            foreach (Control c in P_Menues.Controls)
            {
                if (c.Equals(P_MenuBarContainer)) continue;
            ...
            if (SP_Spliter.Orientation == Orientation.Vertical) P_MenuBarContainer.Width = maxSize;
            try
            {
                SP_Spliter.SplitterDistance = maxSize;
                SP_Spliter.Refresh();
            }
            catch
            {
        ...
        void TcpServerForm_FormClosed(object sender, FormClosedEventArgs e)
        {
            if (_conn != null) _conn.Close();
        }
