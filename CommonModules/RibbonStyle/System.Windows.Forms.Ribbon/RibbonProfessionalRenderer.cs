...
namespace System.Windows.Forms
{
		 ...
		 : RibbonRenderer
	{
		...
		public void DrawUpDownButtons(Graphics g, RibbonUpDown b)
		{
			if (b.UpButtonPressed)
				DrawButtonPressed(g, b.UpButtonBounds, Corners.None);
			else if (b.UpButtonSelected)
				DrawButtonSelected(g, b.UpButtonBounds, Corners.None);
			else
				DrawButton(g, b.UpButtonBounds, Corners.None);

			if (b.DownButtonPressed)
				DrawButtonPressed(g, b.DownButtonBounds, Corners.None);
			else if (b.DownButtonSelected)
				DrawButtonSelected(g, b.DownButtonBounds, Corners.None);
			else
		...
		public override void OnRenderOrbDropDownBackground(RibbonOrbDropDownEventArgs e)
		{
			...
			if (e.RibbonOrbDropDown.ContentRecentItemsCaptionBounds.Height > 0)
			{
				...
				e.Graphics.DrawString(e.RibbonOrbDropDown.RecentItemsCaption, new Font(e.Ribbon.Font.FontFamily, e.Ribbon.Font.Size, FontStyle.Bold), Brushes.DarkBlue, cb, sf);
			}
		...
		public void DrawOrbNormal(RibbonRenderEventArgs e)
		{
			using (GraphicsPath path = RoundRectangle(e.ClipRectangle, 2, Corners.North))
			{
				...
				//Border
				using (Pen p = new Pen(ColorTable.OrbButtonBorderDark))
				{
					e.Graphics.DrawPath(p, path);
				}
				...
				{
					using (Pen p = new Pen(ColorTable.OrbButtonMedium))
					{
		...
		/// <param name="e"></param>
		/// <param name="button"></param>
		public void DrawOrbSelected(Graphics g, Rectangle bounds, Corners corners)
		{
			if (bounds.Height <= 0 || bounds.Width <= 0) return;
		...
		public override void OnRenderRibbonItem(RibbonItemRenderEventArgs e)
		{
			...
			else if (e.Item is RibbonUpDown)
			{
				...
				DrawUpDownButtons(e.Graphics, t);
				#endregion
			}
		...
		public override void OnRenderScrollbar(Graphics g, Control ctl)
		{
			...
			else
			{
				...
				{
					DrawButtonPressed(g, list.ButtonDownBounds, Corners.SouthEast);
				}
				else if (list.ButtonDownSelected)
				{
					DrawButtonSelected(g, list.ButtonDownBounds, Corners.SouthEast);
				}
				else
				{
					DrawButton(g, list.ButtonDownBounds, Corners.None);
