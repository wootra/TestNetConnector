...
namespace System.Windows.Forms
{
	...
	public class RibbonToolTip : ToolTip
	{
		...
		/// </summary>
		public RibbonToolTip(Ribbon Owner)
		{
			try
			{
				_Owner = Owner;
				...
				this.Popup += new PopupEventHandler(ToolTip_Popup);
		...
		void ToolTip_Draw(object sender, DrawToolTipEventArgs e)
		{
			try
			{
				...
				RibbonToolTipRenderEventArgs eTip = new RibbonToolTipRenderEventArgs(_Owner, e.Graphics, e.Bounds, TipText);
				eTip.Color = Color.Black;
				eTip.Font = _Font;

				_Owner.Renderer.OnRenderToolTipBackground(eTip);
				//e.DrawBackground();
				...
				if (_TextRectangle.Width > 0 && _TextRectangle.Height > 0)
				{
					...
					eTip.Text = TipText;
					_Owner.Renderer.OnRenderToolTipText(eTip);
				}
