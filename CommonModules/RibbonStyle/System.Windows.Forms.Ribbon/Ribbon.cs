...
namespace System.Windows.Forms
{
		 ...
		 : Control
	{
		...
		{
			get { return _orbStyle; }
			set { _orbStyle = value; UpdateRegions(); Invalidate(); }
		}
		...
		public Rectangle OrbBounds
		{
			get
			{
				...
				else //Kevin Carbis - office 2010 style orb
				{
					//Measure the string size of the button text so we know how big to make the button
					Size contentSize = TextRenderer.MeasureText(OrbText, this.Font);
					...
					{
						return new Rectangle(4, TabsMargin.Top, contentSize.Width + TabsPadding.Left + TabsPadding.Right, TabsPadding.Top + contentSize.Height + TabsPadding.Bottom);
					}
					else if (OrbVisible && RightToLeft == RightToLeft.Yes && CaptionBarVisible)
					{
						return new Rectangle(this.Width - contentSize.Width - TabsPadding.Left - TabsPadding.Right - 4, TabsMargin.Top, contentSize.Width + TabsPadding.Left + TabsPadding.Right, TabsPadding.Top + contentSize.Height + TabsPadding.Bottom);
					}
					else if (RightToLeft == RightToLeft.No)
					{
						return new Rectangle(4, 4, 0, 0);
		...
		internal void UpdateRegions(Graphics g)
		{
			...
			if (RightToLeft == RightToLeft.No)
			{
				...
				else //2010 - no need to offset for the orb
		...
		private void PaintOn(Graphics g, Rectangle clip)
		{
			try
			{
				...
				if (OrbStyle == RibbonOrbStyle.Office_2010 && _expanded == false)
				{
					...
					Pen p = new Pen((Renderer as RibbonProfessionalRenderer).ColorTable.TabBorder);
