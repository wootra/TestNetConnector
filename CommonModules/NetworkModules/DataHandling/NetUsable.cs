    ...
    */
    ...
    private static UInt32 _encode = 0;
        ...
        */
        ...
        {
            //if (typeSize > byteSize) typeSize = byteSize;
            ...
            for (int i = 0; i < byteSize; i++)
            {
                ...
                Buffer.SetByte(dest, buffBase, Buffer.GetByte(source, i + destOffset));
            }
        }
        ...
        public static object ArrayToObject(T[] source, int offset, Type type)
        {
            int rawSize = Marshal.SizeOf(type);
            //if (rawSize > source)) return null;
            GCHandle handle = GCHandle.Alloc(source, GCHandleType.Pinned);
            IntPtr pObj = handle.AddrOfPinnedObject();
            object retObj = Marshal.PtrToStructure(pObj, type);
            handle.Free();
            //Marshal.FreeHGlobal(pObj);
            return retObj;
        }
        
        public static object ArrayToTObjectSwapBySize(Array source, int offset, T obj, int typeSizeForSwap)
        {
            int rawSize = Marshal.SizeOf(typeof(T));
            int numOfItemInObj = rawSize / typeSizeForSwap; // num of elements in obj.
            if (rawSize > source.Length) return null;
            
            byte[] outArr = new byte[rawSize];

            //IntPtr dBuffer;
            offset *= Marshal.SizeOf(source.GetValue(0)); //convert to byte offset

            IntPtr dBuffer = new IntPtr();
            Marshal.StructureToPtr(obj, dBuffer, false);

            byte* dPt = (byte*)dBuffer.ToPointer();
            
            for (int i = 0; i < numOfItemInObj; i++)
            {
                for (int j = typeSizeForSwap - 1; j >= 0; j--)
                {
                    *(dPt + j+ i*typeSizeForSwap) = Buffer.GetByte(source,offset++);
                }
            }

            //GCHandle oHandle = GCHandle.Alloc(outArr, GCHandleType.Pinned);
            //IntPtr oBuffer = oHandle.AddrOfPinnedObject();
            IntPtr oBuffer = Marshal.UnsafeAddrOfPinnedArrayElement(outArr, 0);

            //object retObj = Marshal.PtrToStructure(oBuffer, objType);
            //sHandle.Free();
            return obj;
        }
        ...
        public static Byte[] getPacketArrayFromArgs(params object[] args)
        {
            ...
            for (int i = 0; i < args.Length; i++)
            {
                ...
                else if(t.Equals(typeof(NetworkObject<int>)))
                {
