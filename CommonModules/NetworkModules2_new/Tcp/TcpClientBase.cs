...
namespace NetworkModules
{
    public class TcpClientBase:INetConnector
    {
        ...
        private Dictionary<int, Thread> _recvThreads;
        protected int _id=-1;
        int _threadId = 0;
        TcpClient _server;
        ...
        private Byte[] _tempRecvBuff;
        #endregion

        public TcpClientBase(int id, StreamWriter errorLog=null, Byte[] tempRecvBuff=null):base(){

            init(id);
            if (errorLog != null) _errorLogWriter = errorLog;
            if (tempRecvBuff == null) _tempRecvBuff = new byte[1024000];
            else _tempRecvBuff = tempRecvBuff;
        }

        public TcpClientBase(StreamWriter errorLog = null, Byte[] tempRecvBuff=null)
        {
            init(-1);
            if (errorLog != null) _errorLogWriter = errorLog;
            if (tempRecvBuff == null) _tempRecvBuff = new byte[1024000];
        ...
        public int Available
        {
            get
            {
                return (_server!=null)? _server.Available:0;
            }
        }
        ...
        */
        ...
        public void runRecvThread(){
            if (_recvThreads == null || _recvThreads.IsAlive == false)
            {
                _isEndConnection = false;
                _recvThreads = new Thread(new ThreadStart(this.recvLoop));
                _recvThreads.Start();
            }
            else
            {
        ...
        private void recvLoop()
        {
            ...
            try
            {
                ...
                while (!_isEndConnection)
                {
                    ...
                    try
                    {
                        size = _server.Client.Receive(_tempRecvBuff, totalSize, 1, SocketFlags.None);//일단 1개를 받음..
                        if (_server.Available > 0)
                        {
                            size += _server.Client.Receive(_tempRecvBuff, totalSize + 1, _server.Available, SocketFlags.None);//나머지를 받음.
                        }
                    }
                    catch
                    {
                        if (E_NetError != null) E_NetError(this, new NetworkErrorEventArgs(_id, NetworkErrorEventArgs.NetErrorMsg.FAIL_WHEN_RECV_MSG));
                        break;
                    }
                    totalSize += size;
                    TransferEventArgs args = new TransferEventArgs(_id, TransferEventArgs.TransferMode.Receive, size, totalSize, "");
                    args.TempBuffer = _tempRecvBuff;
                    if(E_OnCanReceive!=null) E_OnCanReceive(this, args);

                    if (args.IsHandled)
                    {
                        if (args.UsedSize>0 && totalSize > args.UsedSize)//직접처리했는데 버퍼에 있는 내용을 다 소비하지 않았다면..
                        {
                            //남은 부분을 앞으로 가져옴..
                            Buffer.BlockCopy(_tempRecvBuff, args.UsedSize, _tempRecvBuff, 0, totalSize - args.UsedSize);
                            totalSize -= args.UsedSize;
                        }
                    }
                    else //직접처리하지 않았으므로, queue에 넣어줌..
                    {
                        _queue.enqueueFrom(_tempRecvBuff, 0, totalSize);
                        totalSize = 0;
                    }
