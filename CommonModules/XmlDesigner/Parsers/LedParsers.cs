...
{
    public class LedParsers
    {
        public static int FromRange(object value, string minCondition, string maxCondition, int trueValue=1, int falseValue=0)
        {
            if (value is int || value is short || value is byte)
            {
                int minInt = Convert.ToInt32(minCondition);
                int maxInt = Convert.ToInt32(maxCondition);
            ...
            else if (value is float || value is double || value is string)
            {
                ...
                double maxInt = Convert.ToDouble(maxCondition);
        ...
        public static int FromCorrectValue(object value, string passValue, int trueValue=1, int falseValue=0)
        {
                ...
                case "FromRange":
                    {
                        ...
                        return FromRange(value, arg[0], arg[1], trueValue, falseValue);
                    }
                case "FromCorrectValue":
                    {
                        ...
                        return FromCorrectValue(value, arg[0], trueValue, falseValue);
                    }
                default:
