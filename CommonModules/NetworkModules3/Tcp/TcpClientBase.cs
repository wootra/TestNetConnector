    ...
    public class TcpClientBase:INetConnector
    {
        ...
        */
        ...
        public int readFromNet(Byte[] buff, int offset, int byteSize, out SocketError error)
        {
            ...
            {
                if (_clients.ContainsKey(_threadId) && _clients[_threadId].Client != null)
                    size = _clients[_threadId].Client.Receive(buff, offset, size, SocketFlags.None, out error);
                else
                    return -1;
        ...
        void RemoveThread(int id)
        {
            if (_clientThreads.ContainsKey(id))
            {
                ...
                {
                    Win32APIs.SendMsgData("RemoveThread:"+e.Message, "LogWindow");
        ...
        private void tryConnection()
        {
            ...
            if (conn != null && conn.Client != null && conn.Connected == true)
            {
                ...
                if (_runRecvThreadWhenConnected)
                {
                    ...
                    if (_isDisposing == false) whenConnectionFailed(id);
...
//            }
           ...
           // _isRecvThreadRunning = false;
            ...
            //_isEndConnection = false;
            FinishAClient();
            if (E_NetError != null && id==_threadId) E_NetError(this, new NetworkErrorEventArgs(_id, NetworkErrorEventArgs.NetErrorMsg.RECV_THREAD_FINISHED));
            //ConnectionEvent(ConnType.Disconnected);
            //close();
