...
namespace ReportTools
{
    public partial class ReportDialog : Form
    {
        ...
        */
        ...
        {
            this.Width = pageWidth+35;
            float rate = 40.0f;
            float wid = pageWidth / rate;
            ReportForm.setSize(wid, wid);
            double colWid = 0;
            for (int i = 0; i < Names.Count; i++)
            {
                ...
                //    txt = ""+i;
                colWid = (columnWidths.ElementAt(i)<0)? 1:(columnWidths.ElementAt(i))/rate;
        ...
        public void AddARow(object[] items)
        {
            ReportForm.addValues(items);

        }
        public void AddARow(List<object> items)
        {
            ReportForm.addValues(items);
        }
        public void AddARow(Dictionary<String,object> items)
        {
            ReportForm.addValues(items);
        }
