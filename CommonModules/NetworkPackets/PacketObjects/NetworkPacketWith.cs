...
namespace NetworkPacket
{
    ...
    public class NetworkPacketWith<T> : ResizableNetworkObject<T>,INetPacket
    {
        ...
        */
        ...
        public List<String> getValuesAsString(Dictionary<int, String> changedValue = null, bool isSwap=true)
        {
            ...
            foreach (int item in _itemListForView)
            {
                ...
                values.Add(getValueAsFormat(item, isSwap));
        ...
        /// 값의 배열을 String리스트로 받아옴.
        /// </summary>
        /// <param name="isSwap">Swap할 것인지 물어봄..</param>
        /// <param name="changedValue">정규화된 값이 아닌 사용자가 원하는 값을 넣고 싶을 때, 항목의 이름과 값의 쌍으로 넣어주면 리스트에서 해당위치에 들어간다.</param>
        /// <returns>Visible로 선택한 모든 값이 리스트로 리턴됨.</returns>
        public List<String> getValuesAsString(bool isSwap, Dictionary<int, String> changedValue = null)
        {
            List<String> values = new List<String>();
            foreach (int item in _itemListForView)
            {
        ...
        public String getValueAsFormat(int item, bool isSwap)
        {
            String format;
        ...
        public Int64 getValueInt(int item, Endians endian = Endians.Little)
        {
            ...
            if (_startByteOffsetInBuffer.ContainsKey(item))//버퍼상에 실제 할당되는 unit
            {
                ...
                bool isSwap = endian == Endians.Big;
                switch (byteSize)
                {
                    case 1:
                        value = TypeArrayConverter.CopyBufferToVariable<byte>(_initBuffer, startOffset, byteSize, unsigned, isSwap);

                        break;
                    case 2:
                        if (unsigned) value = TypeArrayConverter.CopyBufferToVariable<ushort>(_initBuffer, startOffset, byteSize, unsigned, isSwap);
                        else value = TypeArrayConverter.CopyBufferToVariable<short>(_initBuffer, startOffset, byteSize, unsigned, isSwap);
                        break;
                    case 4:

                        if (unsigned) value = TypeArrayConverter.CopyBufferToVariable<uint>(_initBuffer, startOffset, byteSize, unsigned, isSwap);
                        else value = TypeArrayConverter.CopyBufferToVariable<int>(_initBuffer, startOffset, byteSize, unsigned, isSwap);
                        break;
                    case 8:
                        if (unsigned) value = TypeArrayConverter.CopyBufferToVariable<Int64>(_initBuffer, startOffset, byteSize, unsigned, isSwap);
                        else value = TypeArrayConverter.CopyBufferToVariable<Int64>(_initBuffer, startOffset, byteSize, unsigned, isSwap);
                        break;
                }
            ...
            else if (_bitIndex.ContainsKey(item)) //bit 분리된 unit
            {
                ...
                unsigned = _isUnsigned[parent];
                bool isSwap = endian == Endians.Big;

                switch (byteSize)//부모의 값을 먼저 가져와서
                {
                    case 1:
                        value = TypeArrayConverter.CopyBufferToVariable<byte>(_initBuffer, startOffset, byteSize, unsigned, isSwap);
                        break;
                    case 2:
                        value = TypeArrayConverter.CopyBufferToVariable<short>(_initBuffer, startOffset, byteSize, unsigned, isSwap);
                        break;
                    case 4:
                        value = TypeArrayConverter.CopyBufferToVariable<int>(_initBuffer, startOffset, byteSize, unsigned, isSwap);
                        break;
                    case 8:
                        value = TypeArrayConverter.CopyBufferToVariable<Int64>(_initBuffer, startOffset, byteSize, unsigned, isSwap);
                        break;
                }
                ...
                value = trimBit(value, bitStart, bitSize);
