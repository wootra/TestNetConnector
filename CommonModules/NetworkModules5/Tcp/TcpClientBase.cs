...
namespace NetworkModules5
{
    public class TcpClientBase:NetConnector
    {
       ...
       // private Timer _writeTimer;
        ...
        public TcpClientBase(int id, int headerSize, StreamWriter errorLog = null)
            :base(headerSize)
        {
        ...
        public TcpClientBase(int headerSize, StreamWriter errorLog = null)
            :base(headerSize)
        {
            _tempRecvBuffer = new byte[1024000];
            init(-1);
        ...
        public TcpClientBase(Byte[] tempRecvBuffer, int headerSize, StreamWriter errorLog = null)
            : base(headerSize)
        {
        ...
        */
        ...
        public void ConnectToServer(string server, int port, int connectionTimeout, int readTimeout, int writeTimeout)
        {
            ...
            setServerInfo(server, port, readTimeout, writeTimeout);
            if (isConnected() == false)//_connectionState== ConnType.Disconnected)
            {
        ...
        */
        ...
        public void reconnect()
        {
            ...
            {
                int currentId = _threadId;
                if (_isPermarnentConnecting == false)
                {

                    _threadId++;
                    StartConnection(_connectionTimeout, _runRecvThreadWhenConnected, false);
                }
                RemoveThread(currentId);
                //Disconnect(); //접속을 끊고..
        ...
        public override int write(Byte[] buff, int offset, int size)
        {
            try
            {
                ...
                {
                    if(_connectionState== ConnType.Connected) Disconnect();
                    return -1;// throw e;
                }
                else
                {
        ...
        void StartConnection(int connectionTimeoutWithMs=2000, bool runRecvThreadWhenConnected=false, bool relaeaseThread=true)
        {
            _connectionState = ConnType.Disconnected;
            if (relaeaseThread) releaseThreads();
        ...
        */
        ...
        protected virtual void NetworkError(NetworkErrorEventArgs e) {
            RunNetErrorEvent(_id, e.error, e.errorStr);
        }
        ...
        void whenConnectionFailed(int id)
        {
            ...
            if (_isPermarnentConnecting)
            {
                ...
                {
                    Thread.Sleep(1000);
                    if(_isDisposing==false) StartConnection(_connectionTimeout, _runRecvThreadWhenConnected, false);
                }
            }
            else
            {
  ...
  //          {
                ...
                while (id==_threadId)//현재 thread가 최근 thread가 아니면 나감..
                {
                   // int size = 0;
                    ...
                    bool? result = InRecvLoop(_tempRecvBuffer, socket, _id, ref _totalSize, ref errorCount, _queue);
...
//            }
            ...
            //if (_isPermarnentConnecting) StartConnection(2000, true, false);
            //else Disconnect();
            //ConnectionEvent(ConnType.Disconnected);
            //close();
