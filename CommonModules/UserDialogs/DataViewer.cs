...
namespace UserDialogs
{
    public partial class DataViewer : Form
    {
        ...
        public DataViewer(String initDir)
        {
            InitializeComponent();
            _initDir = initDir;
            V_Data.U_ShowIndex = true;
        ...
        public void SetTities(params String[] titles){
       ...
       public void clear()
        ...
        private void CheckInteger()
        {
            ...
            try
            {
                ...
                reader.ReadLine();//title..버림.
                beforeLine = reader.ReadLine();//첫라인
                tokens = beforeLine.Split(",".ToCharArray());
                _checking = true;
                iBefore = long.Parse(tokens[_selectedCol]);// V_Data.Rows[1].Cells[col].Value as String;
                ...
                {
                    lineIndex++;//2부터 시작
                        value = line.Split(",".ToCharArray())[_selectedCol];// V_Data.Rows[i].Cells[_selectedCol].Value as String;
                        
                        try
                        {
                            iVal = long.Parse(value);
                            ...
                            if ((iVal - iBefore) != iDiff)
                            {
                                if (this.InvokeRequired)
                                {
                                    ...
                                    this.Invoke(aal, true, lineIndex, line);
                                }
                                else
                                {
                                    addALine(false, lineIndex - 1, beforeLine);
                                    addALine(true, lineIndex, line);
                                }
                                errorIndex++;
                        ...
                        {
                            AddLog("error/line:" + (lineIndex + 1) + ", 형식이 맞지 않음:" + value.ToString());
                        }
                        beforeLine = line;
                }
        ...
        private void CheckTimeStamp()
        {
            ...
            //DateTime time = new DateTime(0), beforeTime = new DateTime(0);
            long time, beforeTime;

            long iDiff;
            long.TryParse(T_Diff.Text, out iDiff);
            string line, beforeLine;
            string[] tokens;
            ...
            try
            {
                ...
                try
                {
                    ...
                    tokens = beforeLine.Split(",".ToCharArray());
                    _checking = true;
                    timestamp = tokens[_selectedCol];// V_Data.Rows[1].Cells[col].Value as String;
                    beforeTime = getTime(timestamp);
                    int errorIndex = 0;
                    //for (int i = 2; (_checking == true && i < V_Data.Rows.Count); i++)
                    int lineIndex = 1;
                    while ((line = reader.ReadLine()) != null && _checking == true)
                    {
                        lineIndex++;//2부터 시작
                        timestamp = line.Split(",".ToCharArray())[_selectedCol];// V_Data.Rows[i].Cells[_selectedCol].Value as String;

                        try
                        {
                            time = getTime(timestamp);

                            if ((time - beforeTime) != iDiff)
                            {
                                if (this.InvokeRequired)
                                {
                                    addalinedele aal = addALine;
                                ...
                                errorIndex++;
                                AddLog(""+errorIndex+"/line:" + (lineIndex) + "(" + (time - beforeTime) + ")");
                            }
                            if(lineIndex%30==0) L_Line.Text = lineIndex.ToString();
                            beforeTime = time;
                        }
                        catch (Exception ee)
                        {
                            AddLog("error/line:" + (lineIndex) + ", 형식이 맞지 않음:" + timestamp.ToString());
                        }
                        beforeLine = line;
        ...
        delegate void addalinedele(bool isChecked, long lineIndex, String line);
        void addALine(bool isChecked, long lineIndex, String line)
        {
                ...
                if (_fileReader != null)
                {
                    ...
                    else
                    {
                        ...
                        while (lineToForward-- > 0)
                        {
                            ...
                            if (line != null) addALine(false, _nowLine++, line);
                            else break;
