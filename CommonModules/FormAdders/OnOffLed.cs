...
namespace FormAdders
{
    public partial class OnOffLed : UserControl
    {
        ...
        public void setStateInvoke(ConnType connType)
        {
            lock (this)
            {
                if (connType == ConnType.Connected) this.pic.BackgroundImage = _onImage;
                else if (connType == ConnType.Connecting) this.pic.BackgroundImage = _midImage;
                else this.pic.BackgroundImage = _offImage;
            }
        }
