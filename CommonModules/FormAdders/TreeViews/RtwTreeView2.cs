...
namespace FormAdders
{
    ...
    public class RtwTreeView2 : TreeView
    {
        ...
        public delegate void ContextMenuClickHandler(Object sender, String text, int index, TriStateTreeNode selectedNode, object MenuItem);
        ...
        ImageList DisabledStateImageList = new ImageList();
        public RtwTreeView2()
            : base()
        {
            ...
            DrawTrianlgeStyle = true;

            StateImageList = null;
            CheckBoxImageList = null;
            ...
            base.Margin = new Padding(10, 10, 10, 10);
        }

        internal virtual void ClearInfos()
        {
        ...
        [Browsable(true)]
        public new ImageList CheckBoxImageList
        {
            get
            {
            ...
            set
            {
                ...
                else
                {
                    ...
                    this.DisabledStateImageList = value;
                }
            }
        }
        ...
        protected override void OnMouseDoubleClick(MouseEventArgs e)
        {
            ...
            if (info.Node != null && (info.Location & TreeViewHitTestLocations.StateImage)>0)// .ToString() == "StateImage")
            {
        ...
        bool _drawTriangleStyle = true;
        public bool DrawTrianlgeStyle
        {
            get { return _drawTriangleStyle; }
            set {
                _drawTriangleStyle = value;
                /*
                if (_drawTriangleStyle)
                {
                    this.DrawMode = TreeViewDrawMode.OwnerDrawAll;
        ...
        public enum ShowNodeModes { ImageCheckText = 0, CheckImageText };
        ShowNodeModes _showNodeMode = ShowNodeModes.ImageCheckText;
        public ShowNodeModes ShowNodeMode
        {
            get { return _showNodeMode; }
            set { _showNodeMode = value; }
        }
        ...
        void DrawNode(Graphics g, DrawTreeNodeEventArgs e)
        {
            Rectangle Bounds = e.Bounds;
            bool IsVisible = e.Node.IsVisible;
            int StateImageIndex = e.Node.StateImageIndex;
            ...
            int goLeft = (ShowPlusMinus) ? 35 : 55;// (ShowCheckBoxes && ShowIcons) ? 40 : (ShowCheckBoxes) ? 20 : (ShowIcons) ? 20 : 0;
            int leftStart = Bounds.X - goLeft;
            int plusminusStart = Bounds.X - 55;
            int stringStart = leftStart;
            int iconSize = (ShowIcons) ? 20 : 0;
            stringStart += iconSize;
            int checkBoxSize = (ShowCheckBoxes) ? 20 : 0;
            stringStart += checkBoxSize;
            int rightEnd = Bounds.X + Bounds.Width + goLeft + iconSize + checkBoxSize;

            Rectangle fullBack = new Rectangle(leftStart, Bounds.Y, this.Width - leftStart, Bounds.Height);
            g.FillRectangle(new SolidBrush(BackColor), fullBack);


            //Rectangle nodeRect = new Rectangle(Bounds.X - checkWidth, Bounds.Y, this.Width - Bounds.X - checkWidth, Bounds.Height);// NodeBounds(e.Node);
            //Rectangle nodeRect = new Rectangle(Bounds.X - goLeft-2, Bounds.Y-2, this.Width- Bounds.X + goLeft+4, Bounds.Height+4);// NodeBounds(e.Node);

            //CellFunctions.DrawLensBack(null, nodeRect, g, Color.Transparent, true, this.Enabled);
            //CellFunctions.DrawPlainBack(null, nodeRect, g, Color.White, Color.White, false, this.Enabled);
            //EasyGridViewCollections.CellFunctions.DrawLensBack(nodeRect, g, Color.Transparent, true);
            //g.FillRectangle(new SolidBrush(Color.SkyBlue), nodeRect);
            /*
         if (ShowCheckBoxes) //Checked 가 true인지 false인지에 따라 정해짐..
             g.DrawImage(this.CheckBoxImageList.Images[StateImageIndex], new Point(Bounds.X - 40, Bounds.Y));//Bounds.Location);// Rectangle.Inflate(e.Bounds, 22, 0));
         else if (ShowIcons) //Node가 Expand인지 Elapsed인지에 따라 정해짐.
             g.DrawImage(this.CheckBoxImageList.Images[ImageIndex],Bounds.X - 40, Bounds.Y, 40, Bounds.Height);//Bounds.Location);// Rectangle.Inflate(e.Bounds, 22, 0));
         */
            Rectangle StringRect = new Rectangle(leftStart, Bounds.Y, rightEnd, Bounds.Height - 2);// NodeBounds(e.Node);
            //CellFunctions.DrawLensBack(null, StringRect, g, Color.Transparent, true, this.Enabled);

            if (State == (TreeNodeStates.Selected | TreeNodeStates.Focused))
            {
                if (this.Enabled) Selection.DrawSelection(g, StringRect);

                //CellFunctions.DrawPlainBack(null, StringRect, g, Color.White, Color.Blue, true, this.Enabled);
            }
            else
            {
                //g.DrawRectangle(Pens.White, StringRect);
                //CellFunctions.DrawPlainBack(null, StringRect, g, Color.White, Color.White, false, this.Enabled);
            }

            if (ShowNodeMode == ShowNodeModes.ImageCheckText)
            {
                if (ShowIcons)
                {
                    OnDrawNodeImage(e, leftStart, Bounds.Y, g);
                }
                if (ShowCheckBoxes)
                {
                    int imgIndex = (SelectedImageIndex == 1) ? 1 : (Nodes.Count > 0) ? 0 : 0;

                    g.DrawImage(this.CheckBoxImageList.Images[StateImageIndex], new Point(leftStart + iconSize, Bounds.Y));
                }
            }
            else
            {
                ...
                if (ShowCheckBoxes)
                {
                    ...
                    g.DrawImage(this.CheckBoxImageList.Images[StateImageIndex], new Point(leftStart, Bounds.Y));
                }
                if (ShowIcons)
                {
                    try
                    {
                        OnDrawNodeImage(e, leftStart + checkBoxSize, Bounds.Y, g);
                    }
                    catch (Exception ex)
                    {
                        throw;
            ...
            if (_drawTriangleStyle)
            {
                if (ShowPlusMinus)
                {
                    Rectangle plusMinusRect = new Rectangle(plusminusStart, Bounds.Y, 20, 20);
                    g.FillRectangle(Brushes.White, plusMinusRect);
                    if (SelectedImageIndex == 1) g.DrawImage(this.ImageList.Images[1], new Point(plusminusStart, Bounds.Y));// Bounds.Location);
                    else if (Nodes.Count > 0) g.DrawImage(this.ImageList.Images[2], new Point(plusminusStart, Bounds.Y));// Bounds.Location);
                    else g.DrawImage(this.ImageList.Images[0], new Point(plusminusStart, Bounds.Y));// Bounds.Location);
                }
            }
            //g.DrawImage(this.ImageList.Images[SelectedImageIndex], new Point(Bounds.X - 20 - checkWidth, Bounds.Y));// Bounds.Location);
            if (this.Enabled)
                g.DrawString(Text, nodeFont, Brushes.Black, new Point(stringStart, Bounds.Y));// Bounds);//Rectangle.Inflate(e.Bounds, 42, 0));
            else
                g.DrawString(Text, nodeFont, Brushes.Gray, new Point(stringStart, Bounds.Y));// Bounds);//Rectangle.Inflate(e.Bounds, 42, 0));
            /*
                //Rectangle rect = new Rectangle(new Point(Bounds.X +22, Bounds.Y), new Size(20, 20));
                if (ShowCheckBoxes) //Checked 가 true인지 false인지에 따라 정해짐..
                    g.DrawImage(this.CheckBoxImageList.Images[StateImageIndex], new Point(Bounds.X - 40, Bounds.Y));//Bounds.Location);// Rectangle.Inflate(e.Bounds, 22, 0));
                else if (ShowIcons) //Node가 Expand인지 Elapsed인지에 따라 정해짐.
                    g.DrawImage(this.CheckBoxImageList.Images[ImageIndex], new Point(Bounds.X - 40, Bounds.Y));//Bounds.Location);// Rectangle.Inflate(e.Bounds, 22, 0));
                    

                                    if (SelectedImageIndex == 1) g.DrawImage(this.ImageList.Images[1], new Point(Bounds.X - 20 - checkWidth, Bounds.Y));// Bounds.Location);
                                    else if (Nodes.Count > 0) g.DrawImage(this.ImageList.Images[2], new Point(Bounds.X - 20 - checkWidth, Bounds.Y));// Bounds.Location);
                                    else g.DrawImage(this.ImageList.Images[0], new Point(Bounds.X - 20 - checkWidth, Bounds.Y));// Bounds.Location);
                                    //g.DrawImage(this.ImageList.Images[SelectedImageIndex], new Point(Bounds.X - 20 - checkWidth, Bounds.Y));// Bounds.Location);
                                    g.DrawString(Text, nodeFont, Brushes.Black, new Point(Bounds.X - checkWidth, Bounds.Y));// Bounds);//Rectangle.Inflate(e.Bounds, 42, 0));
            ...
            if (this.Enabled)
            {
                ...
                {
                    //Rectangle nodeRect = new Rectangle(Bounds.X - checkWidth, Bounds.Y, this.Width - Bounds.X - checkWidth, Bounds.Height);// NodeBounds(e.Node);
                    Rectangle nodeRect = new Rectangle(Bounds.X - checkWidth, Bounds.Y, this.Width + checkWidth, Bounds.Height);// NodeBounds(e.Node);
            ...
            // Use the default background and node text.
                ...
                {
                    //                Rectangle nodeRect = new Rectangle(Bounds.X-checkWidth, Bounds.Y, Bounds.Width + 50, Bounds.Height); NodeBounds(e.Node);
                    //Rectangle nodeRect = new Rectangle(Bounds.X - checkWidth, Bounds.Y, this.Width - Bounds.X - checkWidth, Bounds.Height);
                    Rectangle nodeRect = new Rectangle(Bounds.X - checkWidth, Bounds.Y, this.Width + checkWidth, Bounds.Height);
                    //NodeBounds(e.Node);

                    //g.FillRectangle(new SolidBrush(this.BackColor), nodeLineRect);
                    //Rectangle nodeRect = new Rectangle(Bounds.X - checkWidth, Bounds.Y, this.Width - Bounds.X - checkWidth, Bounds.Height);// NodeBounds(e.Node);
                    ...
                    //if (ShowCheckBoxes) g.DrawImage(this.CheckBoxImageList.Images[StateImageIndex], new Point(Bounds.X - 40, Bounds.Y));//Bounds.Location);// Rectangle.Inflate(e.Bounds, 22, 0));
                    if (ShowCheckBoxes) //Checked 가 true인지 false인지에 따라 정해짐..
                        g.DrawImage(this.CheckBoxImageList.Images[StateImageIndex], new Point(Bounds.X - 40, Bounds.Y));//Bounds.Location);// Rectangle.Inflate(e.Bounds, 22, 0));
                    else if (ShowIcons) //Node가 Expand인지 Elapsed인지에 따라 정해짐.
                        g.DrawImage(this.CheckBoxImageList.Images[ImageIndex], new Point(Bounds.X - 40, Bounds.Y));//Bounds.Location);// Rectangle.Inflate(e.Bounds, 22, 0));
            ...
            else
            {
                ...
                if (ShowCheckBoxes) //Checked 가 true인지 false인지에 따라 정해짐..
                    g.DrawImage(this.DisabledStateImageList.Images[StateImageIndex], new Point(Bounds.X - 40, Bounds.Y));//Bounds.Location);// Rectangle.Inflate(e.Bounds, 22, 0));
                else if (ShowIcons) //Node가 Expand인지 Elapsed인지에 따라 정해짐.
                    g.DrawImage(this.CheckBoxImageList.Images[ImageIndex], new Point(Bounds.X - 40, Bounds.Y));//Bounds.Location);// Rectangle.Inflate(e.Bounds, 22, 0));
                        
                //if (ShowCheckBoxes) g.DrawImage(this.DisabledStateImageList.Images[StateImageIndex], new Point(Bounds.X - 40, Bounds.Y));//Bounds.Location);// Rectangle.Inflate(e.Bounds, 22, 0));

                g.DrawImage(this.ImageList.Images[SelectedImageIndex], new Point(Bounds.X - 20 - checkWidth, Bounds.Y));// Bounds.Location);

                g.DrawString(Text, nodeFont, Brushes.Gray, new Point(Bounds.X - checkWidth, Bounds.Y));// Bounds);//Rectangle.Inflate(e.Bounds, 42, 0));
        ...
        protected virtual void OnDrawNodeImage(DrawTreeNodeEventArgs e, int x, int y, Graphics g)
        {
            if (e.Node.SelectedImageIndex == 1) g.DrawImage(this.ImageList.Images[1], new Point(x, y));// e.Node.Bounds.Location);
            else if (e.Node.Nodes.Count > 0) g.DrawImage(this.ImageList.Images[2], new Point(x, y));// e.Node.Bounds.Location);
            else g.DrawImage(this.ImageList.Images[0], new Point(x, y));// e.Node.Bounds.Location);
        }
        ...
        protected override void OnMouseDown(MouseEventArgs e)
        {
            ...
            if(clickedNode!=null)
            {
                ...
                _selectedTreeNode = clickedNode as TriStateTreeNode;
            }
            
            int checkBoxWidth = (ShowCheckBoxes) ? 0 : 20;
            System.Windows.Forms.TreeViewHitTestInfo info = this.HitTest(e.X + checkBoxWidth, e.Y);
            if (info.Node != null && (info.Location & TreeViewHitTestLocations.StateImage)>0) //checkbox
            {
        ...
        public new TreeViewHitTestInfo HitTest(int x, int y)
        {
            ...
            TreeViewHitTestLocations locations = 0;// TreeViewHitTestLocations.None;
            TreeViewHitTestInfo info;
            if (clickedNode == null)
            {
                if (locations == 0) locations = TreeViewHitTestLocations.None;
                info = new TreeViewHitTestInfo(clickedNode, locations);

                return info;
            }
            int goLeft = ((this).ShowPlusMinus) ? 35 : 55;// (ShowCheckBoxes && ShowIcons) ? 40 : (ShowCheckBoxes) ? 20 : (ShowIcons) ? 20 : 0;
            int leftStart = clickedNode.Bounds.X - goLeft;
            int plusminusStart = clickedNode.Bounds.X - 55;
            int stringStart = leftStart;
            int iconSize = ((this).ShowIcons) ? 20 : 0;
            stringStart += iconSize;
            int checkBoxSize = ((this).ShowCheckBoxes) ? 20 : 0;
            stringStart += checkBoxSize;
            int rightEnd = clickedNode.Bounds.X + clickedNode.Bounds.Width + goLeft + iconSize + checkBoxSize;

            
            if (y < clickedNode.Bounds.Y) locations |= TreeViewHitTestLocations.AboveClientArea;
            else if (y > clickedNode.Bounds.Y + clickedNode.Bounds.Height) locations |= TreeViewHitTestLocations.BelowClientArea;
            if (x < clickedNode.Bounds.X) locations |= TreeViewHitTestLocations.LeftOfClientArea;
            else if (x > this.Bounds.X + clickedNode.Bounds.Width) locations |= TreeViewHitTestLocations.RightOfClientArea;

            if ((this).ShowNodeMode == RtwTreeView2.ShowNodeModes.CheckImageText)
            {
                if (x > leftStart && x < leftStart + checkBoxSize) locations |= TreeViewHitTestLocations.StateImage;
                else if (x > plusminusStart && x < plusminusStart + 20) locations |= TreeViewHitTestLocations.PlusMinus;
                else if (x > leftStart + checkBoxSize && x < leftStart + checkBoxSize + iconSize) locations |= TreeViewHitTestLocations.Image;
                else if (x > plusminusStart + 20 && x < leftStart) locations |= TreeViewHitTestLocations.Indent;
                else if (x > stringStart && x < rightEnd) locations |= TreeViewHitTestLocations.Label;
            }
            else
            {
                if (x > leftStart + iconSize && x < leftStart + iconSize + checkBoxSize) locations |= TreeViewHitTestLocations.StateImage;
                else if (x > plusminusStart && x < plusminusStart + 20) locations |= TreeViewHitTestLocations.PlusMinus;
                else if (x > leftStart && x < leftStart + iconSize) locations |= TreeViewHitTestLocations.Image;
                else if (x > plusminusStart + 20 && x < leftStart) locations |= TreeViewHitTestLocations.Indent;
                else if (x > stringStart && x < rightEnd) locations |= TreeViewHitTestLocations.Label;
            }
            if (locations == 0) locations = TreeViewHitTestLocations.None;
        ...
        protected override void OnMouseClick(MouseEventArgs e)
        {
            if (_isCheckBoxClicked)
            {
                ...
                {
                    if ((info.Location & TreeViewHitTestLocations.StateImage)>0)//.ToString() == "StateImage")
                    {
        ...
        */
        ...
        public virtual TriStateTreeNode AddRoot(String text)
        {
        ...
        public TriStateTreeNode AddChild(TriStateTreeNode parent, String text, string name){
            TriStateTreeNode node = new TriStateTreeNode(text, name);
        ...
        {
            TriStateTreeNode node = new TriStateTreeNode(text, text);
            parent.Nodes.Add(node);
            return node;
        }
        ...
        TriStateTreeNode _contextMenuBase = null;
        public TriStateTreeNode ContextMenuBase
        {
            get { return _contextMenuBase; }
        }

        void DoAction(Actions action, TreeNode passedNode){
            if (_isCheckBoxClicked) return;
            SelectedNode = passedNode;
            ...
            if((action & Actions.ContextMenuOpened)>0){ //ContextMenu가 있으면 연다.
                _contextMenuBase = node;
                if (node.ContextMenu != null && node.ContextMenu.MenuItems.Count>0) //소속된 contextmenu가 있고, item들이 있을 때
                {
                   // node.ContextMenu.Show(this, this.PointToClient(Control.MousePosition));
                    //소속된 contextMenu가 있으면 이 처리를 하지 않아도 자동으로 보인다.
                }
                else //소속된 contextMenu가 없다면
                {
                    if (node.Nodes.Count == 0)//자식노드가 없는 노드일 때..  
                    {
                        if (node.Parent != null && node.Parent.ChildContextMenu.MenuItems.Count > 0)//부모에서 자식의 contextMenu가 정해져 있다면
                        {
                            node.Parent.ChildContextMenu.Show(this, this.PointToClient(Control.MousePosition));
                        }
                        else if (U_ContextMenuEndNode != null) //공통 contextMenu가 있을 때
                        {
                    ...
                    else //자식노드가 있는 노드라면
                    {
                        ...
                        else if (U_ContextMenuParentNode != null)
                        {
        ...
        protected override void OnNodeMouseClick(TreeNodeMouseClickEventArgs e)
        {
            ...
            /// 현 상태를 state안에 넣어줌.
            bool? state =   (_selectedTreeNode.CheckState == CheckState.Checked) ? true : 
                            (_selectedTreeNode.CheckState == CheckState.Unchecked) ? false : 
                            /*else */ (bool?)null;
        ...
        /// <param name="tooltip">메뉴에서 보여질 툴팁</param>
        /// <param name="inputGesture">바로가기 버튼을 지정할 수 있다. Ctrl+C 와 같이 적는다.</param>
        public void AddContextMenuItemParentNode(TriStateTreeNode node, String text, ContextMenuClickHandler eventHandler = null, ContextMenu baseMenu = null, String tooltip = null, String inputGesture = null)
        {
            if (baseMenu == null)
            {
                if (node.ContextMenu == null) node.ContextMenu = new System.Windows.Forms.ContextMenu();
                baseMenu = node.ContextMenu;// U_ContextMenuParentNode;
            }
            else _contextMenuParentNode = baseMenu; //직접 지정해 줄 경우, 레퍼런스를 바꾸어준다.
            MenuItem item = new MenuItem();
            ContextMenuItems.Add(item);
        ...
        void  ParentNodeMenuClicked(object sender, EventArgs e)
        {
            ...
            if (_contextMenuParentClickHandlers.ContainsKey(item)) _contextMenuParentClickHandlers[item].Invoke(ContextMenuBase, item.Text.ToString(), index, _selectedTreeNode, item);
            else if (E_ContextMenuParentClicked != null) E_ContextMenuParentClicked(ContextMenuBase, item.Text.ToString(), index, _selectedTreeNode, item);
        }
        ...
        /// <param name="inputGesture">바로가기 버튼을 지정할 수 있다. Ctrl+C 와 같이 적는다.</param>
        /// <returns></returns>
        public ContextMenu AddContextMenuItemEndNode(String text, ContextMenuClickHandler eventHandler = null, ContextMenu baseMenu = null, String tooltip = null, String inputGesture = null)
        {
            ...
            if (eventHandler != null) _contextMenuEndClickHandlers.Add(item, eventHandler);
        ...
        /// <param name="tooltip">메뉴에서 보여질 툴팁</param>
        /// <param name="inputGesture">바로가기 버튼을 지정할 수 있다. Ctrl+C 와 같이 적는다.</param>
        public ContextMenu AddContextMenuItemEndNode(TriStateTreeNode parentNode, String text, ContextMenuClickHandler eventHandler = null, ContextMenu baseMenu = null, String tooltip = null, String inputGesture = null)
        {
            if (baseMenu == null) baseMenu = parentNode.ChildContextMenu;
            MenuItem item = new MenuItem();
        ...
        void EndNodeMenuClicked(object sender, EventArgs e)
        {
            MenuItem item = sender as MenuItem;
            int index = ContextMenuItems.IndexOf(item);
            if (_contextMenuEndClickHandlers.ContainsKey(item)) _contextMenuEndClickHandlers[item].Invoke(ContextMenuBase, item.Text.ToString(), index, _selectedTreeNode, item);
            else if(E_ContextMenuEndClicked!=null) E_ContextMenuEndClicked(ContextMenuBase, item.Text.ToString(), index, _selectedTreeNode, item);
        }
        ...
        */
        ...
        public List<TriStateTreeNode> FindCheckedEndNodes()
        {
            ...
            if (ShowCheckBoxes == false) return chList;
        ...
        {
            _contextMenuEndClickHandlers.Clear();
            _contextMenuParentClickHandlers.Clear();
            _relativeObject.Clear();
            this.Nodes.Clear();
        }
    ...
    public class NodeInfo
    {
        ...
        public bool IsRefreshed { get { return _isRefreshed; } set { _isRefreshed = value; } }
        DrawTreeNodeEventArgs _args;
        public DrawTreeNodeEventArgs args {
            get { return _args; }
            set
            {
                _args = value;
