...
namespace FormAdders
{
    public partial class PairList : UserControl
    {
        ...
        Dictionary<String, Control> valueDic = new Dictionary<string, Control>();
        Boolean isInit = true;
        Timer scrollTimer = null;
        EventHandler ScrolltimerTickEventHandler=null;

        public void makeInit()
        {
            isInit = false;
            TextBox initBox = new TextBox();
            initBox.Text = "insert some Control on This";
            AddRow("testName", initBox);
            isInit = true;
        }
        ...
        public String[] U_InitValuesPair
        {
            ...
            set
            {
                ...
                for (int i = 0; i < value.Length; i++)
                {
                    ...
                    else strValue = _initControlType.ToString();
                    try
                    {
                        AddRow(strName, getControlByText(strValue));
                    }
                    catch(Exception e)
                    {
                        MessageBox.Show("PairList.U_InitValuesPair.set:" + e.Message);
                        return;
                    }
                }
                readyToRedraw();
        ...
        #region U_InitControlType
        public enum ControlType { TextBox, CheckBox, ComboBox, NumberTextBox, ThreeImageLed, ImgButton, OnOffLed, PictureBox };
        
        ControlType _initControlType = ControlType.TextBox;

        [SettingsBindable(true)]
        [Browsable(true)]
        [Bindable(true)]
        public ControlType U_InitControlType
        {
            get { return _initControlType; }
            set { _initControlType = value; }
        }
        #endregion

        #region getControlByText
        public Control getControlByText(String controlType)
        {
            switch (controlType)
            {
                case "TextBox":
                ...
                default:
                    throw new Exception(controlType+": 그런 이름의 컨트롤은 U_InitValuesPair명령에서는 지원하지 않습니다.\r\n 새로운 타입의 컨트롤을 추가하시려면 addRow명령을 이용하여 코드로 추가해 주세요");
        ...
        public int U_FixedValuePosition { 
            ...
            set {
                ...
                if (U_ValuePosition == ValuePositionMode.FixedPosition)
                {
                    readyToRedraw();
                    reDrawAll();
                }
        ...
        Color _textBackColor = Color.White;
        [SettingsBindable(true)]
        [Browsable(true)]
        public Color U_TextBackColorForTransparent
        {
            ...
            set
            {
                ...
                if (_backColor.Equals(Color.Transparent))
                {
        ...
        [SettingsBindable(true)]
        [Browsable(true)]
        public Boolean U_IsColWidthFixed
        {
            get { return _isColWidthFixed; }
            set
            {
        ...
        [SettingsBindable(true)]
        [Browsable(true)]
        public Boolean U_IsScrollEnabled
        {
        ...
        #region U_TextWidth
        public int _textWidth = 100;
        [SettingsBindable(true)]
        [Browsable(true)]
        public int U_TextWidth { get { return _textWidth; } set { _textWidth = value; } }
        #endregion

        #region U_TextHeight
        public int _textHeight = 14;
        [SettingsBindable(true)]
        [Browsable(true)]
        public int U_TextHeight { get { return _textHeight; } set { _textHeight = value; } }
        #endregion
        ...
        public Control getValueTextBox(String name)
        {
            return valueDic[name];
        }

        public List<Control> getValueTextBoxList()
        {
            return values;
        }
        ...
        public void AddRow(String name, Control addingControl)
        {
            int y = 0;
            int x = 0;
            setValueX();
            ...
            names.Add(nameLabel);
            nameString.Add(name);
            values.Add(addingControl);
            valueDic.Add(name, addingControl);
            
            //BackPanel.SuspendLayout();
            BackPanel.Controls.Add(nameLabel);
            BackPanel.Controls.Add(addingControl);

            if (_isUnderLine)
            {
                UserControl underLine = new UserControl();
                underLine.Location = new Point(0, y + addingControl.Height);
