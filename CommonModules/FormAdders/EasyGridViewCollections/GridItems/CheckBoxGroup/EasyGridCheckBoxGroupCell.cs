...
namespace FormAdders.EasyGridViewCollections
{
    public class EasyGridCheckBoxGroupCell:DataGridViewCell, IEasyGridCell
    {
       ...
       // Rectangle _bounds;
        ...
        void DrawCheckBox(Graphics g, int x, int y, CheckBox radio, int textOffset, int imageOffset, Brush fontColor)
        {
        ...
        public new object Value
        {
            ...
            set
            {
                if (value is EasyGridCellInfo && (value as EasyGridCellInfo).ItemType == ItemTypes.CheckBoxGroup)
                {
                    EasyGridCellInfo info = value as EasyGridCellInfo;
                    SetValue(info);
                }else if (value is ICollection<int>)
                {
                    SetValue(value as ICollection<int>);
                }
                else if (value == null)
                {
                    SetValue((ICollection<int>)null);
                }
                else
                {
                    throw new InvalidTypeException(value, new Type[] { typeof(EasyGridCellInfo), typeof(ICollection<int>) });// Exception("테이블 [" + this.Name + "]의 입력 멤버타입이 틀립니다. row:" + row.Index + ", cell:" + colIndex + ", trying value:" + (value as String) + "/ 원래 타입:String[],int[], null");
                }
            }
        }
    ...
    public class EasyGridCheckBoxGroupCollection:ICollection<CheckBox>,IEnumerable<CheckBox>{
        ...
        public void SetCheckedItems(ICollection<int> selected)
        {
            if (selected != null)
            {
        ...
        public void SetCheckedItems(int selectedIndex1, params int[] selected)
        {
            List<int> indice = new List<int>();
            indice.Add(selectedIndex1);
            if(selected!=null && selected.Length>0) indice.AddRange(selected);
            SetCheckedItems(indice);
        }
        ...
        public ICollection<int> GetValue()
        {
            ...
            {
                if (this[i].Checked) selected.Add(i);
            }
            return selected;
        }
        ...
        public void SetValue(ICollection<int> value)
        {
            if (value != null)
            {
                for (int i = 0; i < this.Count; i++)
                {
                    ...
                    if (value.Contains(i)) this[i].Checked = true;
